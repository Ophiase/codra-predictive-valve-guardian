import time
from dataclasses import dataclass

import pandas as pd
from sklearn.metrics import accuracy_score, f1_score

from processor.data.build_dataset import build_dataset, build_raw_dataset, split_dataset
from processor.model import (
    K_FFT_FEATURES,
    ClassifierProtocol,
)
from processor.train.model_candidates import CANDIDATES, MODEL_CANDIDATES, RawOrFeatures


@dataclass(frozen=True)
class ModelMetric:
    accuracy: float
    f1: float
    train_time: float
    predict_time: float


def evaluate_model(
    model: ClassifierProtocol,
    X_train: pd.DataFrame,
    y_train: pd.Series,
    X_test: pd.DataFrame,
    y_test: pd.Series,
) -> ModelMetric:
    start_train = time.time()
    model.fit(X_train, y_train)
    train_time = time.time() - start_train

    start_pred = time.time()
    y_pred = model.predict(X_test)
    pred_time = time.time() - start_pred

    acc = accuracy_score(y_test, y_pred)
    f1 = f1_score(y_test, y_pred)
    return ModelMetric(
        accuracy=acc, f1=f1, train_time=train_time, predict_time=pred_time
    )


def evaluate_all() -> pd.DataFrame:
    results = []

    for config in CANDIDATES:
        start_feat = time.time()
        if config.X == RawOrFeatures.RAW:
            X, y = build_raw_dataset()
        else:
            X, y = build_dataset(
                feature_order=config.feature_order,
                fft_feature=config.fft,
                k_fft_features=K_FFT_FEATURES,
            )
        feat_time = time.time() - start_feat

        X_train, y_train, X_test, y_test = split_dataset(X, y)

        for name, model in MODEL_CANDIDATES.items():
            print(
                f"Config: data={config.X}, feature_order={config.feature_order}, "
                f"fft={config.fft}, model={name}"
            )
            metric = evaluate_model(model, X_train, y_train, X_test, y_test)
            results.append({
                "data": config.X,
                "feature_order": config.feature_order,
                "fft": config.fft,
                "model": name,
                "accuracy": metric.accuracy,
                "f1": metric.f1,
                "total_execution_time": feat_time + metric.predict_time,
                "feature_time": feat_time,
                # "train_time": train_time,
                "predict_time": metric.predict_time,
            })

    df_results = pd.DataFrame(results)
    df_results_sorted = df_results.sort_values(
        by=["f1", "accuracy", "total_execution_time"], ascending=[False, False, True]
    )

    return df_results_sorted


if __name__ == "__main__":
    df_results = evaluate_all()
    print(df_results)
    # df_results.to_csv("naive_best_model_estimation_results.csv", index=False)

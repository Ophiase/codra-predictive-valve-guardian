...
def evaluate_model(model: object, X_train: pd.DataFrame, y_train: pd.Series, X_test: pd.DataFrame, y_test: pd.Series) -> Tuple[float, float, float, float]:
...
def evaluate_all() -> pd.DataFrame:
    ...
    df_results_sorted = df_results.sort_values(
        by=["f1", "accuracy", "total_execution_time"],
